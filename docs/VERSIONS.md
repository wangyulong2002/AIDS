...
> （服务数、内存口径、未 bump 的版本号）。根因不是不认真，而是**没有一处权威地方写着「这套版本长什么样」**。
> 本文件就是这个锚点。
...
| 文件 | 当前版本 | 上次修订 | 本轮修订原因 |
|------|----------|----------|--------------|
| [PRD.md](PRD.md) | **v1.5** | 2026-09-24 | 移除全文的工期与工作量内容（§11 里程碑去周期列、§15 #6 去排期数字）；保留 v1.4 的协作与范围决定 |
...
| 9 | 全量档内存口径 | **宿主可用 ≥6GB（8GB 舒适）**；容器空载实测 ≈1.5GB（默认档 ≈0.9GB） | PRD §13 / §15 / README 分档表 / compose 头部 | `grep -rn '≥ 6GB\|≥6GB' docs/PRD.md deploy/README.md deploy/docker-compose.yml` | ⬜ 人工 |
| 10 | 三服务端口 | 主业务 **8080** / AI **8000** / Mock **8081**（前端另有 80，不计入） | PRD §5.3 / API 头部 / 各 Dockerfile EXPOSE | `grep -h EXPOSE deploy/app/backend.Dockerfile deploy/app/ai.Dockerfile deploy/app/mock.Dockerfile` | ✅ |
| 11 | 部署踩坑条数 | **11** | deploy/README.md | `grep -cE '^\*\*[0-9]+\.' deploy/README.md` | ✅ |
| 12 | Java 残留 | **0 处活跃引用**（仅允许带 v1.3 注记的历史/对照说明） | 全仓 | `grep -rniE '\bjava\b\|spring\|mybatis\|jvm\|nacos\|flyway\|actuator' docs/ deploy/ \| grep -v 'application/javascript'` 后人工判定 | ⬜ 人工 |
...
| 版本节点 | 日期 | 动了什么 | 连带数字变化 |
|----------|------|----------|--------------|
| v1.0 / v1.0 | 2026-09-19 前 | 初始设计冻结 | 11 服务、37 表 |
| **v1.2 / v2.0** | 2026-09-19 | **移除 Elasticsearch**，商品搜索改 MySQL 全文索引（`ngram`） | 服务 11→10；撤销 `product.changed` topic |
...
| v1.3 收尾 | 2026-09-21 | 按 `文档质量锐评.md` 修复：内存口径统一、`data_scope` 退化定义、成本上界、本版本矩阵 | 无表/服务数变化；DD·seed·mock_schema 版本补齐 v1.1 |
...
| **v1.4 / v2.2** | 2026-09-23 | **协作与范围口径定稿**（PRD §15 #6）：采纳「**1 人 + AI**」协作、范围暂不裁剪；同批落地 C9~C12 契约测试与三服务骨架（详见 HANDOFF §0.1） | 无表 / 字段 / 枚举 / 服务数变化；任务总数仍为 **97** |
| **v1.5 / v2.2** | 2026-09-24 | **移除全文的工期与工作量内容**：PRD §11 里程碑去「周期」列、§15 #6 去排期数字；TASKS 的「工作量对账」改为 **§范围与协作口径**（只保留范围/裁剪顺序/硬底线/分工/并行口径）；门禁脚本去掉「16 周残留」检查 | 无表 / 字段 / 枚举 / 服务数变化；**§二 常量 13 → 12 条**（删除第 9 条「总工期」，其后 10/11/12/13 顺次改为 9/10/11/12） |
...
> 跨文档常量（服务数/任务数/踩坑条数/端口/内存口径）、`.env` 重复键与 compose 变量覆盖、
...
| `docs/API.md` 错误码或接口 | 核对 PRD §5.4 错误码分段；本表 #8~#10 无变化也要过一遍 |
