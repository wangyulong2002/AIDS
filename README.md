...
> **当前阶段**：T1 进行中（工程骨架 + 首块业务代码）
> **代码量**：BE-03 鉴权模块已落地（JWT 双 Token / RS256 / JWKS / 刷新轮换与吊销 / 依赖注入拦截）；其余模块为分组占位
...
| 层 | 触发 | 内容 | 耗时上限（**门禁自身**的运行时长，与项目排期无关） |
|---|---|---|---|
...
> 文档一致性门禁（`docs/tools/gen_data_dictionary.py --check`，34 项）**必须是第一步**：
